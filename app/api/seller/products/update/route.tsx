...
export async function POST(req: Request, res: Response) {
    ...
    if (session?.user.role == "SELLER") {
        ...
        const product_id = posted_data.get("product_id") as string;
        ...
        const cities_to_add: any = cities.map(city => {
            return {
                cityId: city.id,
        ...
        if (update_product) {
            ...
            const delete_previous_cities = await prisma.productCity.deleteMany({
                where: {
                    productId: product_id
                }
            })
