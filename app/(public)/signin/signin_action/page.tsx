    ...
    useEffect(() => {
        ...
        const get_session = async () => {
            ...
            if (my_session?.user) {
                ...
                // console.log(user_response)
                // console.log(user_response.data.welcomeEmailSent)

                if (user_response.data.welcomeEmailSent === false) {
                    // send welcome email
                    const form_data = new FormData();
                    form_data.set("user_id", user_response.data.id);
                    form_data.set("name", user_response.data.name);
                    form_data.set("email", user_response.data.email);
                    const fetch_sendWelcomeEmail = await fetch("/api/email/welcome", { method: "POST", body: form_data })
                    const response_sendWelcomeEmail = await fetch_sendWelcomeEmail.json()
                    console.log(response_sendWelcomeEmail)
                }
